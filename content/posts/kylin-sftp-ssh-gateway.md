...
date = "2026-08-27T05:31:41+00:00"
description = "记录银河麒麟 V10 上 SFTP、双网卡、隔离网闸和老版 JSch 算法协商问题的排查过程。"
tags = [
  "记录",
]
+++

这是一份围绕银河麒麟 V10、SFTP、双网卡和安全隔离网闸的排查记录。问题表面上看是“Windows 客户端连不上 SFTP”，但实际涉及多个层次：操作系统版本识别、`sshd` 服务、SFTP 子系统、监听地址、网闸两侧地址关系、文件交换策略，以及老版本 JSch 客户端与 OpenSSH 之间的算法协商。
...
如果设备采用“协议剥离 → 数据摆渡 → 协议重建”的工作方式，普通 SSH 加密会话可能不在支持范围内。此时即便 TCP 探测成功，也可能在应用层被拒绝。
...
如果设备只支持 FTP，可以考虑使用它原生支持的文件交换方式，但这会改变安全边界，必须按现场安全要求评估，不能为了临时连通就随意降级。
...
## 10. 老版本 JSch 的 SSH 算法协商失败

另一条排查线索来自 Wireshark 抓包：客户端使用的是 JSch `0.1.54`。双方完成版本交换和 `KEXINIT` 算法列表交换后，客户端发送了断开消息，原因类似：

```text
com.jcraft.jsch.JSchException: Algorithm negotiation fail
```

这比简单说“加密算法不兼容”更准确。SSH 协商包含多类算法：
...
只要其中某一类没有交集，会话就可能在认证前失败。

### 10.1 HostKeyAlgorithms 可能是关键
...
双方缺少共同的主机密钥算法，常见的最小兼容方向是让服务端额外提供：

```conf
HostKeyAlgorithms +ssh-rsa
```

但这只是基于抓包和算法列表的判断，实际应用前仍要查看服务器真实算法列表和客户端配置。
...
但如果抓包已经证明 KEX、Cipher 和 MAC 都有共同算法，就不应为了保险再扩大它们。应先只测试：
...
- [SSH 协商失败分析（ChatGPT）](https://chatgpt.com/share/6a8fc7c4-b364-83e8-93d8-f0dd250463f0)
- [查看国产 Linux 系统版本（千问）](https://qianwen.my.cn/share/chat/d64bed3b8a9043fbba66bbdec9091720)
